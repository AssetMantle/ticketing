...
function loadMoreNFTs(collectionId) {
    ...
    if ($(".noNFT").length === 0) {
        let route = jsRoutes.controllers.SaleController.collectionNFTsPerPage(collectionId, pageNumber);
        $.ajax({
            url: route.url,
            type: route.type,
            async: true,
...
function loadFirstNFTs(collectionId){
    loadFirstNFTBulk('nftsPerPage', jsRoutes.controllers.SaleController.collectionNFTsPerPage(`${collectionId}`, 1));
